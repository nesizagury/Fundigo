...
public class ArtistStatsActivity extends Activity implements GetEventsDataCallback {
    private static final String TAG = "ArtistStatsActivity";
    TextView artistTV;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (GlobalVariables.ALL_EVENTS_DATA.size() == 0) {
            ...
            for (int i = 0; i < GlobalVariables.ALL_EVENTS_DATA.size(); i++) {
                Log.e(TAG, GlobalVariables.ALL_EVENTS_DATA.get(i).getArtist());
                Log.e(TAG, "size " + GlobalVariables.ALL_EVENTS_DATA.size());
                if (GlobalVariables.ALL_EVENTS_DATA.get(i).getArtist().equals("")) {
                    eventsList.add(GlobalVariables.ALL_EVENTS_DATA.get(i));
                }
            }
            calculateStates();
    ...
    private void calculateStates() {
        Log.e(TAG, "size of new list " + eventsList.size());
        int sumIncome = 0;
        int sumTickets = 0;
        int pastEvents = 0;
      ...
      //  StaticMethods.filterEventsByArtist(artistName, eventsList);
        ...
        if (upcomingEvents > 0) {
            ...
            Log.e(TAG,""+(upcomingTicketAvg / upcomingEvents) );
        }
    }
